  ...
  try {
    ...
    if (!festivalName && info.lMonth === 12 && info.lDay >= 29) {
      try {
        ...
        const nextInfo = CALENDAR.solar2lunar(nextD.getFullYear(), nextD.getMonth() + 1, nextD.getDate())
...
export function buildDayData(dateStr, day, isCurrentMonth, y, m, events, holidays) {
  ...
  const primarySubLabel = holidayName || workdayName || lunarInfo.festivalName || lunarInfo.termName || lunarInfo.lunar || ''
  const secondarySubLabel = holidayName || workdayName
  ...
  return {
    ...
    primarySubLabel,
    secondarySubLabel,
    hasRichLabel: !!(holidayName || workdayName || lunarInfo.festivalName || lunarInfo.termName),
    events: dayEvents,
    weekday
  }
}
