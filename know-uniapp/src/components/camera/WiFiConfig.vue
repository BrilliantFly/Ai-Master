<template>
  <view class="wifi-config-container">
          ...
          <view class="wifi-item-left">
            ...
            <view class="wifi-item-info">
              ...
              <text class="wifi-item-detail">
                {{ wifi.secure ? '已加密' : '开放网络' }} | {{ getSignalText(wifi.signalStrength) }}
              </text>
            </view>
          </view>
    ...
    <view class="config-result" v-if="configResult">
      <view class="result-card" :class="{ success: configResult.success, failed: !configResult.success }">
        ...
        <button class="result-action retry" v-else @tap="retryConfig">
          重新配网
        </button>
      </view>
    </view>
...
import { WiFiConfigManager, type WifiConfigData, type ConfigStatus } from '@/utils/camera/wifiConfig';
...
function startConfig() {
  ...
  wifiConfigManager.startConfig(wifiData)
    .then((success) => {
      configResult.value = {
        success: true,
