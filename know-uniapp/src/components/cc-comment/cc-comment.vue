            ...
            <view class="reply_text">
                <template v-if="Object.keys(replyTemp).length">
                    ...
                    <img
                        ...
                        :src="
                            replyTemp.item2
                                ? replyTemp.item2.userAvatar
                                : replyTemp.item1.userAvatar
                        "
                    />
                    <span class="text_main">{{
                        replyTemp.item2 ? replyTemp.item2.userNe : replyTemp.item1.userName
                    }}</span>
                </template>
...
function treeTransForm(data) {
    ...
    newData.forEach((item, i) => {
        item.owner = item.userId === props.myInfo.userId // 是否为当前登陆用户 可以对自己的评论进行删除 不能回复
        item.author = item.userId === props.userInfo.userId // 是否为作者 显示标记
        map[item.id] = item
    })
...
// 发起新评论
const isNewComment = ref(false) // 是否为新评论
...
const commentPlaceholder = ref('说点什么...') // 输入框占位符
...
function sendClick({ item1, index1, item2, index2 } = replyTemp) {
    ...
    if (isNewComment.value) {
        params = {
            ...
            replyId: null, // 被回复评论id
            replyName: null // 被回复人名称
        }
    } else {
        // 回复评论
        params = {
            id: Math.random(), // 评论id
            parentId: item?.parentId ?? item.id, // 父级评论id
            replyId: item.id, // 被回复评论id
            replyName: item.userName // 被回复人名称
        }
    }
    params = {
        ...params,
        userId: props.myInfo.userId, // 用户id
        userName: props.myInfo.userName, // 用户名
        userAvatar: props.myInfo.userAvatar, //  用户头像地址
        userContent: commentValue.value, // 用户评论内容
        isLike: false, // 是否点赞
        likeCount: 0, // 点赞数统计
        createTime: '刚刚', // 创建时间
    ...
    emit('replyFun', { params }, (res) => {
        ...
        if (isNewComment.value) {
            ...
            // 回复
            const c_data = dataList.value[index1]
...
function delConfirmFun({ item1, index1, item2, index2 } = delTemp) {
    ...
    if (index2 >= 0) {
        emit(
            'deleteFun',
            { params: [c_data.children[index2].id], mode: props.deleteMode },
            (res) => {
                uni.hideLoading()
                emit('update:tableTotal', props.tableTotal - 1)
...
function expandReplyFun({ item1, index1 }) {
    const csLen = dataList.value[index1].childrenShow.length
...
function shrinkTxtShow({ item1, index1 }) {
    return (
        item1.childrenShow?.length >= 2 && item1.children.length - item1.childrenShow.length === 0
    )
}
// 收起更多评论
