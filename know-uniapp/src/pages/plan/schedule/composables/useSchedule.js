...
import { generateWeeks, calcMonthlyStats, quadrantColor, getCurrentWeekDays, formatYYYYMMDD } from '@/components/calendar-grid/calendar-utils.js'
...
export function useSchedule() {
  ...
  const filteredDayEvents = computed(() => {
    const events = currentQuadrant.value === 0
      ? dayEvents.value
  ...
  const weeks = computed(() => {
    return generateWeeks(currentYear.value, currentMonth.value, calendarSelected.value, holidays.value)
  })
  ...
  const init = async () => {
    ...
    await fetchHolidays()
    getCategoryList({}).then(res => { categories.value = res || [] }).catch(() => {})
    // 默认选中今天
    const now = new Date()
  ...
  return {
    ...
    weeks, currentWeekDays, monthlyStats, holidays,
    quadrantColor,
    init, fetchCalendarMonthly, onMonthSwitch, onDateTap,
    fetchDayEvents, handleCheck, toggleCollapse, filterByQuadrant
  }
}
