    ...
    <view class="plan-home-page">
            ...
            <view class="hero-top">
                <view class="hero-welcome">
                    ...
                    <view class="hero-greeting">
                        ...
                        <text class="sub">{{ currentYear }} 年 {{ currentMonth }} 月 · 高效管理每一天</text>
                    </view>
                </view>
            ...
            <view class="asset-right">
                <view class="asset-item">
                    <view class="num num-flex" :style="{ color: 'var(--color-success)' }">
                        <span class="metric-icon green sm">✅</span> {{ scheduleStats.completedCount }}
                    </view>
        ...
        <view class="content">
            ...
            <view class="carousel-wrap premium-fade-in premium-d1">
                <swiper class="carousel-swiper" :indicator-dots="false" :autoplay="true" :interval="3500" :circular="true" @change="onCarouselChange">
                    <swiper-item v-for="(slide, idx) in carouselSlides" :key="idx" class="carousel-slide" :style="{ background: slide.bg }">
                ...
                <view class="carousel-dots">
                    <view v-for="(_, idx) in carouselSlides" :key="idx" class="carousel-dot" :class="{ active: carouselIndex === idx }"></view>
                </view>
            </view>

            <!-- ===== Notice Strip (todayHighlights) ===== -->
            <view v-if="todayHighlights.length" class="notice-strip premium-fade-in premium-d2" @tap="go('/pages/plan/schedule/index')">
                <span class="n-icon">📢</span>
                <view class="n-wrap">
                    <view class="n-scroll" :style="{ transform: 'translateY(-' + (noticeIdx * 16) + 'px)' }">
                        <span v-for="(item, idx) in noticeList" :key="idx" class="n-text">{{ item.text }}</span>
                    </view>
                </view>
            ...
            <view class="section-hdr premium-fade-in premium-d2">
                ...
                <span class="more premium-more-link" @tap="go('/pages/plan/stats/index')">全部 <span>→</span></span>
            </view>
            <view class="quick-grid premium-fade-in premium-d2">
                <view v-for="(item, idx) in quickItems" :key="idx" class="quick-item" @tap="go(item.path)">
            ...
            <view class="section-hdr premium-fade-in premium-d4">
                ...
                <span class="more premium-more-link" @tap="go('/pages/plan/stats/index')">查看全部 <span>→</span></span>
            </view>
            <view class="recommend-scroll premium-fade-in premium-d4">
                <view class="recommend-card" @tap="go('/pages/plan/schedule/index')">
                    <view class="recommend-thumb" style="background:linear-gradient(135deg,#1a1a2e,#6366f1);">
                ...
                <view class="recommend-card" @tap="go('/pages/plan/habit/index')">
                    <view class="recommend-thumb" style="background:linear-gradient(135deg,#0f3443,#22b573);">
                        <span class="r-icon">📋</span>
                ...
                <view class="recommend-card" @tap="go('/pages/plan/focus/index')">
                    <view class="recommend-thumb" style="background:linear-gradient(135deg,#2d1b69,#a855f7);">
                        <span class="r-icon">📊</span>
                ...
                <view class="recommend-card" @tap="go('/pages/plan/stats/index')">
                    <view class="recommend-thumb" style="background:linear-gradient(135deg,#78350f,#f0a020);">
                        <span class="r-icon">🔭</span>
            ...
            <view class="section-hdr premium-fade-in premium-d5">
                ...
                <span class="more premium-more-link" @tap="go('/pages/plan/focus/index')">更多 <span>→</span></span>
            </view>
            <view class="tools-grid premium-fade-in premium-d5">
                <!-- 番茄专注 -->
                <view class="tool-card" @tap="go('/pages/plan/focus/index')">
                    <view class="t-top">
                        <view class="t-icon" style="background:linear-gradient(135deg,#f97316,#fb923c);">🍅</view>
                ...
                <view class="tool-card" @tap="go('/pages/plan/habit/index')">
                    <view class="t-top">
                        <view class="t-icon" style="background:linear-gradient(135deg,#22b573,#34d399);">🎯</view>
                ...
                <view class="tool-card" @tap="go('/pages/plan/schedule/index')">
                    <view class="t-top">
                        <view class="t-icon" style="background:linear-gradient(135deg,#6366f1,#818cf8);">📋</view>
                        <span class="t-title">日程统计</span>
                        <span class="t-tag">今日</span>
                    </view>
                    <span class="t-desc">{{ scheduleStats.completedCount }}/{{ scheduleStats.totalCount }} 已完成</span>
                    <view class="t-bar">
                ...
                <view class="tool-card" @tap="go('/pages/plan/stats/index')">
                    <view class="t-top">
                        <view class="t-icon" style="background:linear-gradient(135deg,#a855f7,#c084fc);">📊</view>
            ...
            <view class="section-hdr premium-fade-in premium-d6">
                ...
                <span class="more premium-more-link" @tap="go('/pages/plan/habit/index')">更多 <span>→</span></span>
            </view>
...
const carouselSlides = [
    { icon: '🏆', title: 'AI 智能监控升级', desc: '实时分析 · 异常告警 · 7x24h 守护', bg: 'linear-gradient(135deg,#6366f1,#8b5cf6)' },
    { icon: '📋', title: '计划管理 3.0 上线', desc: '甘特图 · 日程 · 打卡 · 目标追踪', bg: 'linear-gradient(135deg,#d4956b,#e8b88a)' },
    { icon: '📊', title: '数据分析工具上新', desc: '经营看板 · 报表导出 · 智能分析', bg: 'linear-gradient(135deg,#22b573,#4dd499)' }
]

const quickItems = [
    { icon: '📅', label: '日程计划', bg: 'linear-gradient(135deg,#6366f1,#818cf8)', path: '/pages/plan/schedule/index' },
    { icon: '🎯', label: '习惯打卡', bg: 'linear-gradient(135deg,#22b573,#34d399)', path: '/pages/plan/habit/index' },
    { icon: '🍅', label: '番茄专注', bg: 'linear-gradient(135deg,#f97316,#fb923c)', path: '/pages/plan/focus/index' },
    { icon: '📊', label: '统计洞察', bg: 'linear-gradient(135deg,#a855f7,#c084fc)', path: '/pages/plan/stats/index' },
...
.hero-welcome { display: flex; align-items: center; gap: 10px; }
.hero-avatar {
...
.hero-greeting { color: #fff; }
.hero-greeting .name { font-size: 18px; font-weight: 700; display: block; }
.hero-greeting .sub { font-size: 12px; opacity: 0.75; margin-top: 3px; display: block; }
.hero-icon {
...
.hero-icon:active { background: rgba(255,255,255,0.3); }
.hero-icon .badge {
    position: absolute; top: -3px; right: -3px;
    width: 17px; height: 17px; border-radius: 50%;
    background: var(--color-danger); font-size: 9px;
    display: flex; align-items: center; justify-content: center;
...
.asset-left { flex: 1; }
.asset-label { font-size: 12px; color: var(--color-text-tertiary); letter-spacing: 0.5px; }
.asset-amount { font-size: 24px; font-weight: 800; color: var(--color-text); line-height: 1.3; display: flex; align-items: center; gap: 8px; }
.asset-amount small { font-size: 14px; font-weight: 600; color: var(--color-text-tertiary); }
.asset-right {
    display: flex; gap: 20px;
    padding-left: 16px; border-left: 1px solid var(--color-border-light);
}
.asset-item { text-align: center; }
.asset-item .num { font-size: 16px; font-weight: 700; color: var(--color-text); }
.asset-item .lbl { font-size: 10px; color: var(--color-text-tertiary); margin-top: 2px; }
.asset-item .num-flex { display: flex; align-items: center; gap: 4px; justify-content: center; }
.metric-icon {
...
.metric-icon.red { background: var(--color-danger-soft); color: var(--color-danger); }
.metric-icon.blue { background: var(--color-primary-soft); color: var(--color-primary); }
.metric-icon.sm { width: 22px; height: 22px; font-size: 10px; border-radius: 5px; }
.metric-icon.md { width: 28px; height: 28px; font-size: 12px; border-radius: 6px; }
...
.search-input:active { transform: scale(0.98); }
.search-input .icon { font-size: 14px; color: var(--color-text-tertiary); }
.search-input .text { flex: 1; font-size: 13px; color: var(--color-text-tertiary); }
.search-qr {
...
.search-qr:active { transform: scale(0.92); }
...
.carousel-inner { }
.carousel-inner .c-icon { font-size: 22px; display: block; margin-bottom: 4px; }
.carousel-inner .c-title { font-size: 15px; font-weight: 700; display: block; }
.carousel-inner .c-desc { font-size: 11px; opacity: 0.8; display: block; margin-top: 2px; }
.carousel-dots {
    position: absolute;
    bottom: 8px; left: 50%;
...
.carousel-dot.active { width: 16px; border-radius: 3px; background: #fff; }
...
.section-hdr .title { font-size: 15px; font-weight: 700; color: var(--color-text); }
...
.notice-strip .n-wrap { flex: 1; overflow: hidden; height: 16px; position: relative; }
.notice-strip .n-scroll { transition: transform 0.4s ease; }
.notice-strip .n-text {
    font-size: 12px; color: var(--color-text); line-height: 16px; display: block;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.notice-strip .n-arrow { font-size: 12px; color: var(--color-warning); flex-shrink: 0; }
...
.quick-label { font-size: 11px; color: var(--color-text-secondary); font-weight: 500; }
...
.recommend-thumb .r-icon { font-size: 24px; }
...
.recommend-body { padding: 10px 10px 12px; }
...
.tool-card .t-top { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
.tool-card .t-icon {
...
.tool-card .t-title { font-size: 14px; font-weight: 600; color: var(--color-text); }
.tool-card .t-tag {
...
.tool-card .t-desc { font-size: 11px; color: var(--color-text-secondary); display: block; }
.tool-card .t-bar {
...
/* ===== Swiper overrides ===== */
.carousel-swiper ::v-deep .uni-swiper-dots { display: none; }

/* ===== Hover effects (via JS toggled .hover-active class) ===== */
.hero-icon.hover-active { background: rgba(255,255,255,0.25); }
.asset-card.hover-active { box-shadow: var(--shadow-md); }
.search-input.hover-active { border-color: var(--color-primary); box-shadow: var(--shadow-sm); }
.search-qr.hover-active { border-color: var(--color-primary); box-shadow: var(--shadow-sm); }
.notice-strip.hover-active { box-shadow: var(--shadow-sm); transform: translateY(-1px); }
.quick-item.hover-active .quick-icon { transform: scale(1.1); }
.recommend-card.hover-active { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: transparent; }
.tool-card.hover-active { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: transparent; }
.tool-chip.hover-active { transform: translateY(-1px); box-shadow: var(--shadow-sm); border-color: var(--color-primary); }
</style>
