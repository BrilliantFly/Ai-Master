...
- [ ] 创建 plan_quadrant（四象限表）
- [ ] 创建 plan_type（计划类型表）
- [ ] 创建 plan_info（计划表）
- [ ] 创建 plan_schedule（日程表）
- [ ] 创建 plan_clock_in（打卡任务表）
- [ ] 创建 plan_clock_in_record（打卡记录表）
- [ ] 初始化四象限基础数据
- [ ] 初始化计划类型基础数据
...
CREATE TABLE `plan_clock_in` (
  ...
  `color` varchar(20) DEFAULT NULL,
  `plan_id` bigint DEFAULT NULL,
  `clock_type` tinyint DEFAULT 1,
  `clock_days` varchar(100) DEFAULT NULL,
  `start_time` bigint DEFAULT NULL,
  `end_time` bigint DEFAULT NULL,
  `remind_enabled` tinyint DEFAULT 1,
...
CREATE TABLE `plan_clock_in_record` (
  ...
  `clock_time` bigint DEFAULT NULL,
  `status` tinyint DEFAULT 0,
